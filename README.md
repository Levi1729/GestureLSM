...
GestureLSM is a co-speech gesture generation framework that leverages diffusion models, latent shortcut modeling, and MeanFlow with spatial-temporal reasoning. This repository contains everything needed for training, testing, and running demos, as well as pretrained models and configuration files. This is already done by some guys. I have just took it, fixed errors and modified code to adapt the project to run on my gaming laptop with Nivdia RTX 4060 GPU. There were lot of missing files like the wheel package, .npz simplex models which are missing in the github as they might be deleted during an update. I got all those and made sure that I ran the project successfully.
...
Due to storage constraints, you must manually download the following assets and place them into your local project directory before running the pipeline:

1. **Inference Checkpoint (`new_540_shortcut.bin`):** 
   * Download the primary model checkpoint from the official repository release asset page or your hosted external storage link. 
   * *Target Destination:* `Gesture_LSM/ckpt/new_540_shortcut.bin`(https://drive.google.com/drive/folders/1jbXh3zY-xeT5X4ewGy_HNS-8CUHfzbdI?usp=sharing)

2. **BEAT SMPL Dataset Core (`datasets`):**
   * Request access and download the raw data from the official [BEAT Dataset Repository](https://github.com/zhoubii/BEAT).
   * *Target Destination:* Extract directly into `Gesture_LSM/datasets/` (https://drive.google.com/drive/folders/1hbCgV3JuWYw45a286Fy1vnDfOdtGUtFN?usp=sharing)

3. **SMPL-X 2020 Body Models (`models_smplx_v1_1`):**
   * Register and download the `SMPL-X v1.1 (with FLAME expressions)` package from the official [SMPL-X Project Site](https://smpl-x.is.tue.mpg.de/).
   * *Target Destination:* Extract the files to your dedicated drive directory (e.g., `D:\models_smplx_v1_1\models`). (https://drive.google.com/drive/folders/1MzNbevSVdPOKTis_zTFhbjOZTSfoGji5?usp=sharing)

4. **Pre-trained VQ Weights (`weights`):**
   * Download the required tokenization weights from the [EMAGE Project Storage](https://github.com/pantomatrix/EMAGE) or corresponding repository model hubs.
   * *Target Destination:* Place inside `Gesture_LSM/weights/` (https://drive.google.com/drive/folders/11E5g6CUER-sRoILiPn9ZINhwyj3529Le?usp=sharing)
...
---
## 13. Running
Once every dependency exits. Double check for .npz files, ckpt folder and model weights. For checking those files, you can use Jupyter notebook cells. Once everything is present, run this command after making sure that you have a 4060 GPU (If you have any other gpu, make sure to update code accordingly and check with the cuda versions). Then run this command "python demo.py --config configs_new/shortcut_rvqvae_128.yaml" and then open this local host website link in any browser "http://localhost:7860/" and you get the webpage there and give audio files to get gestures
