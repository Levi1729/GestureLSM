...
def save_checkpoints(save_path, model, opt=None, epoch=None, lrs=None):
    ...
    wavlm_weights = [e for e in model_state_dict.keys() if 'wavlm' in e]
    for e in wavlm_weights:
        model_state_dict.pop(e)

    if lrs is not None:
        states = { 'model_state': model_state_dict,
                'epoch': epoch + 1,
...
def load_checkpoints(model, save_path, load_name='model'):
    ...
    for k, v in states['model_state'].items():
        ...
        if "module" not in k:
            ...
            new_weights[k[7:]]=v
            flag=True
    state_dict_to_load = new_weights if flag else states['model_state']
    try:
        ...
        skipped = sorted({k for k in state_dict_to_load.keys() if k not in compatible_state})
        logger.warning(
            f"Shape mismatch for {len(skipped)} keys when loading {load_name}, skipping: {skipped[:5]}"
        )
        model.load_state_dict(compatible_state, strict=False)
        logger.warning(f"Loaded {load_name} with partial weights; {len(skipped)} keys skipped")
    else:
